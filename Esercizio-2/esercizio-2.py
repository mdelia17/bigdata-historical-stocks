...
prices_RDD = lines_RDD.map(lambda line: [line[0], [ float("{:.5f}".format(float(line[2]))), int(line[6]), line[7]]])
...
sector_year_RDD = join_RDD.map(lambda line: [(line[1][1], datetime.strptime(line[1][0][2], TIMESTAMP_FORMAT).year), [line[1][0][2], line[1][0][2], float("{:.5f}".format(float(line[1][0][0]))), float("{:.5f}".format(float(line[1][0][0])))] ])

sector_year_min_max_RDD = sector_year_RDD.reduceByKey(sector_year_reduction)

sector_year_percent_RDD = sector_year_min_max_RDD.map(lambda line: [line[0], perc(line[1][2],line[1][3])])
...
sector_percent_vol_RDD = ticker_year_RDD.map(lambda line: [(line[0][2], line[0][1]), [line[0][0], perc(line[1][2],line[1][3]), line[0][0], line[1][4]]])
